...
export default function TemplateFillPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Render service status and PDF conversion
  const [renderStatus, setRenderStatus] = useState<'checking' | 'available' | 'unavailable' | null>(null);
  const [isConverting, setIsConverting] = useState(false);
  const [convertedPdfUrl, setConvertedPdfUrl] = useState<string | null>(null);
  const [convertedPdfBuffer, setConvertedPdfBuffer] = useState<Blob | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Check Render service status when document is generated
        checkRenderStatus();
  ...
  const checkRenderStatus = async () => {
    try {
      ...
      const response = await fetch('/api/templates/convert-realtime', {
        method: 'GET'
      });
  ...
  const handleDownload = async () => {
    ...
    // Check Render status if not already checked
    if (renderStatus === null) {
      await checkRenderStatus();
    }
    
    // If Render is available, convert to PDF first
    if (renderStatus === 'available') {
      try {
        setIsConverting(true);
        setConversionError(null);
        
        console.log('🔄 Converting DOCX to PDF in real-time...');
        ...
        if (result.success && result.pdfBuffer) {
          ...
          const base64Data = result.pdfBuffer;
          ...
          a.href = url;
          a.download = `${template?.name?.replace(/[^\w\-\s\.]/g, '').trim().replace(/\s+/g, '-') || 'document'}.pdf`;
          document.body.appendChild(a);
          a.click();
          a.remove();
          window.URL.revokeObjectURL(url);
          
          // Store PDF for "Continue to Order"
          setConvertedPdfUrl(result.pdfUrl || null);
          setConvertedPdfBuffer(pdfBlob);
      ...
      // Render unavailable - show message (don't download Word)
      setError('PDF conversion service is currently unavailable. PDF will be sent to your email in 10 minutes after order completion, or you can try again in 10 minutes.');
    }
  };
  ...
  const handleContinueToOrder = async () => {
    ...
    // If PDF was already converted, upload it and use PDF
    // Otherwise, use Word file
    let fileUrl = generatedWordUrl;
    let isPdf = false;
    
    if (convertedPdfUrl && convertedPdfBuffer) {
      try {
        // Upload PDF to storage
        console.log('📤 Uploading converted PDF to storage...');
        const formData = new FormData();
        formData.append('file', convertedPdfBuffer, `${template.name.replace(/[^\w\-\s\.]/g, '').trim().replace(/\s+/g, '-')}.pdf`);
        ...
        console.error('❌ Error uploading PDF, falling back to Word file:', uploadError);
        // Fall back to Word file
      }
    }
    ...
    const orderData = {
      ...
      wordUrl: isPdf ? undefined : generatedWordUrl,
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {template.allowFreeDownload !== false && (
                <button
                  ...
                  disabled={isConverting || renderStatus === 'checking' || renderStatus === 'unavailable'}
                  className={`px-8 py-3 rounded-lg flex items-center justify-center ${
                    isConverting || renderStatus === 'checking'
                  ...
                  title={
                    isConverting
                      ...
                      ? 'Checking PDF service...'
                      : renderStatus === 'unavailable'
                      ? 'PDF service unavailable'
                      : 'Download PDF (converts from Word document)'
                  }
                >
                  {isConverting ? (
                    <>
              ...
              <button
                ...
                className={`bg-blue-600 text-white px-8 py-3 rounded-lg hover:bg-blue-700 flex items-center justify-center ${
                  convertedPdfUrl ? 'ring-2 ring-green-400' : ''
                }`}
                title={
                  convertedPdfUrl
                    ? 'Continue to order with PDF (ready)'
                    : 'Continue to order. PDF will be generated and sent to your email after payment.'
                }
              >
                {convertedPdfUrl ? (
                  <>
                    Continue to Order (PDF Ready)
                    <svg className="w-5 h-5 ml-2 text-green-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <div className="flex items-start">
                  ...
                  <div className="text-sm text-yellow-800">
                    <p className="font-semibold mb-1">PDF Conversion Service Unavailable</p>
                    <p>PDF will be sent to your email in 10 minutes after order completion, or you can try again in 10 minutes.</p>
                    <p className="mt-1">You can continue with your order using the Word document - it will be automatically converted to PDF after payment.</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                <div className="flex items-start">
                  ...
                  <div className="text-sm text-red-800">
                    ...
                    <p>{conversionError}</p>
                    <p className="mt-1">You can continue with your order - PDF will be generated after payment.</p>
                  </div>
                </div>
              </div>
            )}
            
            {convertedPdfUrl && !conversionError && (
              <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-start">
                  ...
                  <div className="text-sm text-green-800">
                    <p className="font-semibold mb-1">PDF Ready!</p>
                    <p>PDF has been generated and downloaded. Click &quot;Continue to Order&quot; to proceed with the PDF file.</p>
                  </div>
                </div>
              </div>
            )}
            
            {renderStatus === 'available' && !convertedPdfUrl && !conversionError && (
              <div className="mt-4 text-center">
                <p className="text-sm text-gray-600">
                  💡 <strong>Note:</strong> Click &quot;Download PDF&quot; to convert and download your document as PDF, or continue to order and PDF will be generated after payment.
                </p>
              </div>
            )}
