...
import { extractPlaceholders, convertPlaceholdersToDocxtemplater, generateFormSchema } from '@/lib/docxProcessor';
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Convert DOCX to text to extract placeholders
      // For now, we'll use a simple approach - in a real implementation,
      // you might want to use a more sophisticated DOCX parser
      const docxContent = docxBuffer.toString('utf-8');
      
      // Extract placeholders from the content
      placeholders = extractPlaceholders(docxContent);
      ...
      if (placeholders.length > 0) {
        formSchema = generateFormSchema(placeholders);
        
        // Convert placeholders to docxtemplater format and create normalized DOCX
        const normalizedContent = convertPlaceholdersToDocxtemplater(docxContent);
        const normalizedBuffer = Buffer.from(normalizedContent, 'utf-8');
        
        // Upload normalized DOCX to Cloudinary
        normalizedDocxUrl = await uploadToCloudinary(
          normalizedBuffer, 
      ...
      if (manualPlaceholders && manualPlaceholders.length > 0) {
        placeholders = manualPlaceholders;
        formSchema = manualFormSchema || generateFormSchema(placeholders);
      }
    }
