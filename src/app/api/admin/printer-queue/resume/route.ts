...
/**
 * POST /api/admin/printer-queue/resume
 * Resume printer queue processing
 */
export async function POST(_request: NextRequest) {
  try {
    ...
    try {
      const response = await axios.post(joinUrl(printerUrl, '/api/queue/resume'), {}, {
        headers: {
          'X-API-Key': apiKey
        },
        timeout: 5000
      ...
      return NextResponse.json({
        ...
        message: response.data.message || 'Queue resumed successfully'
      });
    } catch (error: any) {
      console.error('Error resuming queue:', error);
      return NextResponse.json({
        success: false,
        error: error.response?.data?.error || error.message || 'Failed to resume queue'
      }, { status: 500 });
    }
  } catch (error) {
    console.error('Error in resume queue endpoint:', error);
    return NextResponse.json({
      success: false,
