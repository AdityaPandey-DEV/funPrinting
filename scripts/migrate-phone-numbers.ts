...
async function migratePhoneNumbers() {
  try {
    ...
    const orders = await Order.find({ 'customerInfo.phone': { $exists: true, $ne: null, $ne: '' } });
    ...
    const adminInfos = await AdminInfo.find({ phone: { $exists: true, $ne: null, $ne: '' } });
    ...
    const pickupLocations = await PickupLocation.find({ contactPhone: { $exists: true, $ne: null, $ne: '' } });
